...
These are the methods to call to end with `select(bool $withPagination = true)`, `count(string $column = "*"): int`, `insert(array $records): int|null`, `update(array $values): int` & `delete: int`, all are pretty self-explanatory.
